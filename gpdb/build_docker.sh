...
# docker build --add-host=gpdbsne:127.0.0.1 --add-host=gpdbsne.localdomain:127.0.0.1 --build-arg GPDB_VERSION="${GPDB_VERSION}",GPDB_HOST="gpdbsne"  -t "${DOCKER_OSS_TAG}" -f  DockerfileOpenSource . &> build.log
#  docker build --add-host=gpdbsne:127.0.0.1 --add-host=gpdbsne.localdomain:127.0.0.1 --build-arg GPDB_VERSION="${GPDB_VERSION}",GPDB_HOST="gpdbsne"  -t "${DOCKER_OSS_TAG}" -f  DockerfileOpenSource .
docker build --add-host=gpdbsne:127.0.0.1 --add-host=gpdbsne.localdomain:127.0.0.1 --build-arg GPDB_VERSION="${GPDB_VERSION}",GPDB_HOST="gpdbsne"  -t "${DOCKER_OSS_TAG}" -f  DockerfileOpenSource .



#docker build --build-arg GPDB_VERSION="${GPDB_VERSION}",GPDB_HOST="gpdbsne"  -t "${DOCKER_OSS_TAG}" -f
